class AnalysisService {
    ...
    analyzeTrend(data) {
      try {
        ...
        const rejections = this.analyzeDetailedRejections(data);
    ...
    analyzeDetailedRejections(data) {
      const keyLevelsObj = this.identifyKeyLevels(data);
      const keyLevels = [
        ...keyLevelsObj.support.map((price) => ({ price, type: "support" })),
        ...keyLevelsObj.resistance.map((price) => ({
          price,
          type: "resistance",
        })),
      ];
  
      const recentRejections = this.findRecentRejections(data, keyLevels);
  
      return {
        keyLevels: keyLevelsObj,
        recentRejections,
        significantRejections:
          this.analyzeSignificantRejections(recentRejections),
        rejectionClusters: this.findRejectionClusters(recentRejections),
      };
    }
    ...
    identifyCandlePatterns(data) {
      const patterns = {
        current: [], // Patterns in the most recent candle(s)
        recent: [], // Patterns in the last few candles
      };
  
      // Analyze the last 3 candles for patterns
      ...
      // Check multiple candle patterns
